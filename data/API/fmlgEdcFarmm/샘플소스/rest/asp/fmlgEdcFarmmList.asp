...
<select name="sType">
	<option value="sLocplc" <%If request("sType")="sLocplc" Then Response.Write("selected") Else Response.Write("") End If%>>지역명</option>
	<option value="sCntntsSj" <%If request("sType")="sCntntsSj" Then Response.Write("selected") Else Response.Write("") End If%>>제목명</option>
	<option value="sThema" <%If request("sType")="sThema" Then Response.Write("selected") Else Response.Write("") End If%>>주제명</option>
</select>
...
<%
	...
	'오퍼레이션 명
	operationName = "fmlgEdcFarmmList"
		...
		For i=0 To cnt-4
		   Set itemNode = items.item(i)
			If NOT itemNode Is Nothing Then
				...
				End If
				'지역명
				If NOT itemNode.SelectSingleNode("adstrdName") is Nothing Then
					adstrdName = itemNode.SelectSingleNode("adstrdName").text
				End If
				'주제
				If NOT itemNode.SelectSingleNode("thema") is Nothing Then
					thema = itemNode.SelectSingleNode("thema").text
				End If
				'소재지
				If NOT itemNode.SelectSingleNode("locplc") is Nothing Then
					locplc = itemNode.SelectSingleNode("locplc").text
				End If
				'연락처
...
%>
		<tr>
			...
			<td align="center"><a href="javascript:;move('<%=cntntsNo%>');">[<%=adstrdName%>]<%=cntntsSj%></a></td>
			<td align="center"><%=thema%></td>
			<td align="center"><%=locplc%></td>
