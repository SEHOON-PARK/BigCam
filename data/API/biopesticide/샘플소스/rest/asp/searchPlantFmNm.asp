...
<%					'과명 서브 리스트 출력
						...
						sub_parameter = sub_parameter & "?apiKey=" & sub_apiKey
					
						sub_targetURL = "http://api.nongsaro.go.kr/service" & sub_parameter
						...
						For i=0 To sub_cnt-1
						   Set itemNode = sub_items.item(i)
							If NOT itemNode Is Nothing Then
								'과명
								If NOT itemNode.SelectSingleNode("fmlNm") is Nothing Then
									sub_fmlNm = itemNode.SelectSingleNode("fmlNm").text
								End If
								'과 일련 번호
								If NOT itemNode.SelectSingleNode("fmlSeqNo") is Nothing Then
									sub_fmlSeqNo = itemNode.SelectSingleNode("fmlSeqNo").text
								End If				
							End If
%>
						<option value="<%=sub_fmlNm%>"<%If Request("fmlNm")=sub_fmlNm Then Response.Write("selected") Else Response.Write("") End If%>><%=sub_fmlNm%></option>
<%
							Set itemNode = Nothing
						Next
%>
				</select>&nbsp;&nbsp;&nbsp;&nbsp;
				학명&nbsp;<input type="text" name="bneNm" value="<%If Request("bneNm") Is Nothing Then Response.Write("") Else Response.Write(Request("bneNm"))%>">&nbsp;&nbsp;&nbsp;&nbsp;
				국명&nbsp;<input type="text" name="yeastNm" value="<%If Request("yeastNm") Is Nothing Then Response.Write("") Else Response.Write(Request("yeastNm"))%>">&nbsp;&nbsp;&nbsp;&nbsp;
				비고&nbsp;<input type="text" name="rm" value="<%If Request("rm") Is Nothing Then Response.Write("") Else Response.Write(Request("rm"))%>">&nbsp;&nbsp;&nbsp;&nbsp;
		    </td>
...
%>
<br>
<div style="TEXT-ALIGN: right"><input type="button" onclick="javascript:location.href='searchPlantFmNm.asp'" value="초기화면"/></div>
</body>
</html>