...
<select name="sType">
	...
	<option value="sAdstrdNm" <?PHP if(isset($_REQUEST["sType"])){ if($_REQUEST["sType"]=="sAdstrdNm") echo "selected"; }?>>지역</option>
</select>
...
<?PHP
		foreach($object->body[0]->items[0]->item as $item){
			...
			//지역
			$adstrdNm = $item->adstrdNm;
			//전화번호
			$telno = $item->telno;
			//쉬는 날
			$restde = $item->restde;
			//영업시간
			$bsnTime = $item->bsnTime;
			//좌석형태
			$seatStle = $item->seatStle;
			//썸네일 이미지
