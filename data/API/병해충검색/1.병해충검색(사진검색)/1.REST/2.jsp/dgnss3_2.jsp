...
<%
	...
	String serviceCode = "SVC08";// 곤충상세조회의 서비스코드(상세한 내용은 Open API 이용안내 참조)
...
%>
   ...
   <%if( !errorCode.equals("")){%>
	...
	<%}else{
		...
		String wrongedCrop = null;
		String shapeInfo = null;
		String quarantineInfo = null;


		try{insectOrder = doc.getElementsByTagName("insectOrder").item(0).getFirstChild().getNodeValue();}catch(Exception e){insectOrder = "";}//곤충목명
		try{insectGenus = doc.getElementsByTagName("insectGenus").item(0).getFirstChild().getNodeValue();}catch(Exception e){insectGenus = "";}//곤충속명
		try{insectFamily = doc.getElementsByTagName("insectFamily").item(0).getFirstChild().getNodeValue();}catch(Exception e){insectFamily = "";}//곤충과명
		try{insectSpecies = doc.getElementsByTagName("insectSpecies").item(0).getFirstChild().getNodeValue();}catch(Exception e){insectSpecies = "";}//곤충종명
		...
		try{insectSubspecies = doc.getElementsByTagName("insectSubspecies").item(0).getFirstChild().getNodeValue();}catch(Exception e){insectSubspecies = "";}//곤충아종명
		try{insectSubgenus = doc.getElementsByTagName("insectSubgenus").item(0).getFirstChild().getNodeValue();}catch(Exception e){insectSubgenus = "";}//곤충아속명
		...
		try{wrongedCrop = doc.getElementsByTagName("wrongedCrop").item(0).getFirstChild().getNodeValue();}catch(Exception e){wrongedCrop = "";}//가해작물
		try{shapeInfo = doc.getElementsByTagName("shapeInfo").item(0).getFirstChild().getNodeValue();}catch(Exception e){shapeInfo = "";}//형태정보
		try{quarantineInfo = doc.getElementsByTagName("quarantineInfo").item(0).getFirstChild().getNodeValue();}catch(Exception e){quarantineInfo = "";}//검역정보
	...
	<%if(enemySize>0){%>
		...
		<%for(int i=0; i<enemySize; i++){%>
			<tr>
				...
				<td><a href="javascript:fncGoEnemy('<%=enemyItems.item(i).getChildNodes().item(7).getFirstChild().getNodeValue()%>');"><%=enemyItems.item(i).getChildNodes().item(3).getFirstChild().getNodeValue()%>(<%=enemyItems.item(i).getChildNodes().item(5).getFirstChild().getNodeValue()%> <%=enemyItems.item(i).getChildNodes().item(6).getFirstChild().getNodeValue()%>)</a></td>
			</tr>
		<%}%>
		</table>
	<%}%>
